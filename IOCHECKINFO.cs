...
{
    public class IOCHECKINFO : IOdata , FuncIOData
    {
        ...
        public IOCHECKINFO(string titulo, string[] checkSelect, color[] backCorral, color[] foreCorral, color[] backtitulo, color[] foretitulo, color[] backSelect, color[] foreSelect, color[] backOption, color[] foreOption, TypeLine line, int columnas, int posX, int posY) : base()
        {
            this._Titulo = titulo;
            ...
            {                
                ...
                if (this._StateEvent == TypeStateIO._ACTIVATED)
                {
                    ...
                    } else
                    {
                        if (tecla.Equals("TAB"))
                        {
                            switch(((this._RolesPermisos >> pos) & 1) == 1)
                            {
                                case (false): this._RolesPermisos = this._RolesPermisos | (1 << pos); break;
                                case (true): this._RolesPermisos = this._RolesPermisos & (~(1 << pos)); break;
                            }
                            script = true;
                        } else
                        {
        ...
        public object GetDataInfo() => (object)this._RolesPermisos;
